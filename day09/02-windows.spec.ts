 ...
 *    cobst page1 = await context1.newPage();
...
test(`Handling windows using sequential approach`, async ({context, page}) => {
    ...
    await newWindow.fill("input#email", "rr7@gamil.com");
 ...
 test.only(`Handling multiple windows`, async ({context, page}) => {
    ...
    await multiplePages.waitForTimeout(3000);
    //Using index
    let webPage:any
    for (let index = 0; index < pages.length; index++) {
        ...
        await webPage.fill("input[placeholder='Search']", "Stephen Shaw");
